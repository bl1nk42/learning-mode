...
Before and after non-trivial code changes, include a framed `★ Insight` block with two or three codebase-specific observations. Every bullet must include an exact ``path/to/file:line`` reference; this visible frame is the log marker.
